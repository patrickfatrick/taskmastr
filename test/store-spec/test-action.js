...
export const testAction = (action, args, state, expectedMutations, done) => {
  ...
  const commit = (name, payload) => {
    ...
    if (payload) {
      // Hacky workaround to prevent problems with timeout IDs
      if (mutation.name === 'UPDATE_DELETE_QUEUE') mutation.payload.id.should.equal(payload.id)
      else mutation.payload.should.deep.equal(payload)
    }
    count++
